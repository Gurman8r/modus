...
namespace ml
{
	...
	public:
		enum : size_t
		{
			ID_null, ID_boolean, ID_integer, ID_unsigned, ID_float, ID_string,
		...
		> using internal_t = typename
			...
			std::conditional_t<std::is_convertible_v<T, string_t>, string_t,
		...
		template <class T> static constexpr size_t index_v{ util::variant_index<variant_t, internal_t<T>>() };

		template <class T> static constexpr bool is_valid_v{ index_v<T> != ID_invalid };
	...
	public:
		...
		variable(self_t && other) noexcept : m_data{} { this->swap(std::move(other)); }
		...
		self_t & operator=(self_t const & other) { this->copy(other); return (*this); }
	...
	public:
		...
		template <class T> ML_NODISCARD auto get() & -> T &
		{
			...
			if constexpr (is_primitive_v<T>) { return std::get<internal_t<T>>(m_data); }
			else { return *std::any_cast<T>(std::addressof(std::get<internal_t<T>>(m_data))); }
		}
		...
		template <class T> ML_NODISCARD operator T() const noexcept { return this->get<T>(); }
	...
	public:
		void copy(self_t const & other) { if (this != std::addressof(other)) { self_t{ other }.swap(*this); } }

		void swap(self_t & other) noexcept { if (this != std::addressof(other)) { m_data.swap(other.m_data); } }
		...
		> decltype(auto) emplace(Args && ... args) noexcept
		{
			...
			if constexpr (!is_object_v<U>)
			{
				if constexpr (is_scalar_v<U>)
				{
					...
					return (T &)(std::get<U>
					(
						m_data = static_cast<U>(ML_forward(std::get<0>(std::forward_as_tuple(ML_forward(args)...))))
					));
				}
				else
				{
					return (U &)(std::get<U>
					(
						m_data = U{ ML_forward(args)... }
					));
				}
			}
			else
			{
				return (T &)(*std::any_cast<T>(std::addressof(std::get<U>
				(
					m_data = std::make_any<T>(ML_forward(args)...))
				)));
			}
		}
