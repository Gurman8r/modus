...
int32_t main(int32_t argc, char * argv[])
{
	...
	struct sandbox final : plugin
	{
		~sandbox() noexcept final {}

		sandbox(plugin_manager * manager) : plugin{ manager }
		{
		}
	...
	> ([&](event const & value)
	{
		...
		case app_enter_event::ID: {
			if (bitmap i{ app.path_to("resource/modus_launcher.png"), false })
			{
		...
		case app_idle_event::ID: {
			...
			app.get_render_context()->execute(
