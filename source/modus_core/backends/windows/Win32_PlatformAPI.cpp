...
namespace ml
{
	...
	std::optional<fs::path> win32_platform_api::open_filename(window_handle window, cstring filter)
	{
		OPENFILENAMEA ofn;
	...
	std::optional<fs::path> win32_platform_api::save_filename(window_handle window, cstring filter)
	{
		OPENFILENAMEA ofn;
		CHAR szFile[MAX_PATH] = { 0 };
		ZeroMemory(&ofn, sizeof(OPENFILENAME));
