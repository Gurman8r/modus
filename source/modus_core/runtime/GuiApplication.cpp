...
namespace ml
{
	...
	gui_application::gui_application(int32 argc, char * argv[], json const & argj, allocator_type alloc)
		...
		, m_render_device	{}
		, m_imgui			{}
		, m_dockspace		{ "##MainDockspace", true }
		, m_active_scene	{}
	...
	void gui_application::on_idle()
	{
		...
		ImGui::PushID(this);
		{
			bool const has_main_menu_bar{ (bool)ImGui::FindWindowByName("##MainMenuBar") };

			ML_flag_write(m_dockspace.WindowFlags, ImGuiWindowFlags_MenuBar, has_main_menu_bar);
			...
			get_bus()->fire<gizmos_event>(this);
		}
		ImGui::PopID();
		ImGui::Render();
